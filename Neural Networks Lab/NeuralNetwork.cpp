...
vector<double> NeuralNetwork::predict(DataInstance instance) {
    ...
    while(!nodeQueue.empty()) {
        ...
        for(auto neighbor : adjacencyList[currentNodeId]) {
            int neighborNodeId = neighbor.first;
            if(!visited[neighborNodeId]) {
                visited[neighborNodeId] = true;
                nodeQueue.push(neighborNodeId);
            }
            visitPredictNeighbor(neighbor.second);
        }
        nodeQueue.pop();
    }
...
bool NeuralNetwork::contribute(double y, double p) {
    ...
    for(int inputNodeId : inputNodeIds) {
        for(auto& neighbor: adjacencyList[inputNodeId]) {
            int neighborNodeId = neighbor.first;
            if(contributions.find(neighborNodeId) == contributions.end()) {
                incomingContribution = contribute(neighborNodeId, y, p);
                contributions[neighborNodeId] = incomingContribution; 
            }
            else {
                incomingContribution = contributions[neighborNodeId];
            }
            visitContributeNeighbor(neighbor.second, incomingContribution, outgoingContribution);  
        }
    }
...
bool NeuralNetwork::update() {
    ...
    while(!nodeQueue.empty()) {
        int currentNodeId = nodeQueue.front();
        nodes[currentNodeId]->bias -= (learningRate * nodes[currentNodeId]->delta);
        nodes[currentNodeId]->delta = 0.0;

        for(auto neighbor = adjacencyList[currentNodeId].begin(); neighbor != adjacencyList[currentNodeId].end(); neighbor++) {
            int neighborNodeId = neighbor->first;
