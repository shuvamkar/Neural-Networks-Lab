...
void Graph::updateNode(int id, NodeInfo n) {
    ...
    updatedNode->activationFunction = n.activationFunction;
    updatedNode->preActivationValue = n.preActivationValue;
    updatedNode->postActivationValue = n.postActivationValue;
    updatedNode->activationDerivative = n.activationDerivative;
    updatedNode->bias = n.bias;
    updatedNode->delta = n.delta;
    nodes.at(id) = updatedNode;
}
...
void Graph::updateConnection(int v, int u, double w) {
    if (v >= adjacencyList.size() || v < 0 || u >= adjacencyList.size() || u < 0) {
        ...
        if(adjacencyList.at(v).find(u) != adjacencyList.at(v).end()) {
            ...
            Connection newConnection(v, u, w);
            adjacencyList.at(v)[u] = newConnection;
        }
    }
}
